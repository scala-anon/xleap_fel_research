...
Runs standalone (``python test_taper.py``) or under pytest. Synthesizes a small
long-form snapshot CSV with a planted hockey-stick taper and checks the whole
chain: store pivot -> lasing detection -> per-time XLEAP timeline.
"""
...
def test_detection_and_timeline(tmp_path) -> None:
    ...
    assert list(frame.columns) == ["xleap_on", "n_und", "taper", "moving"]
